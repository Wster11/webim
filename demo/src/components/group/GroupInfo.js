...
class GroupInfo extends React.Component {
  ...
  handleDissolveGroup = () =>
      this.props.dissolveGroupAsync(this.props.room.groupId);
  ...
  renderGroupOperationMenu = () => {
      ...
      } else if (this.props.group.allowinvites) {
      //成员邀请判断
          return (
              <Menu onClick={this.handleMenuClick}>
                  <Menu.Item key="6">
  ...
  render() {
          ...
          },
          {
              ...
              render: (text, record) => {
                  return ds.length > 0 ? (
                      <Popconfirm
                          title={
                              I18n.t('confirm') + ' ' + I18n.t('removeFromGroupBlackList')
                          }
      ...
      return (
          ...
          >
              ...
              <p className="gray fs-117em">{this.props.room.groupName}</p>
...
export default connect(
    ...
    (dispatch) => ({
        ...
        getGroupBlackListAsync: (groupId) =>
            dispatch(GroupMemberActions.getGroupBlackListAsync(groupId)),
        removeGroupBlockSingleAsync: (groupId, username) =>
            dispatch(
                GroupMemberActions.removeGroupBlockSingleAsync(groupId, username)
            ),
        inviteToGroupAsync: (groupId, users) =>
            dispatch(GroupMemberActions.inviteToGroupAsync(groupId, users)),
        quitGroupAsync: (groupId, username) =>
            dispatch(GroupMemberActions.quitGroupAsync(groupId, username)),
        switchRightSider: ({ rightSiderOffset }) =>
            dispatch(GroupActions.switchRightSider({ rightSiderOffset })),
        newGetGroupInfoAsync: (options) =>
            dispatch(GroupActions.newGetGroupInfoAsync(options))
    })
)(GroupInfo)
